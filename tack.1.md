...
**help**
: Display a friendly help message showing available commands and flags.
...
All template files below _SITEDIR_/templates can be used to create HTML page output by filling them with page content. All templates are expected to be written in the Mustache template language.
...
**Supported markup file extensions:**
- `*.md` (preferred) 
- `*.mkd`
...
- If tack fails with "does not look like a Tack-able site directory", ensure your site directory contains both `content/` and `templates/` subdirectories.

- Use the `-d` flag to enable debug output when troubleshooting build issues.

- Use the `-s` flag to enable strict mode, which will fail if any undefined variables are referenced in templates, helping catch template errors.

- Use `tack list` to verify that tack is detecting and organizing your pages correctly.

- The `serve` command automatically rebuilds when files change, but only checks for changes every 3 seconds.
